...
namespace BoxGame
{
    class GraphicsProvider : IGraphicProvider
	{
		...
		public void ShowUserTurn(IPlayer player)
		{
			...
			Gfx.DrawString(player.GetDisplayName(), new Font(FontFamily.GenericSansSerif, 10), Brushes.Black, new PointF(0, 0));
			m_PrevSize = Gfx.MeasureString(player.GetDisplayName(), new Font(FontFamily.GenericSansSerif, 10));
		}
        ...
        public void DrawImage(Image img, int x, int y) => Gfx.DrawImage(img, x, y);
        ...
        public void DrawString(Point loc, Font font, Color color, string str) => Gfx.DrawString(str, font, new SolidBrush(color), loc);
